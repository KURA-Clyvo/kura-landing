...
1. **Nenhum número ou promessa sem fonte.** Toda afirmação vem da matriz do §0.3 do plano.
   `scripts/check-claims.mjs` roda no build e **falha** se aparecer certificação, acurácia,
   número de cliente, AWS, iPhone prometido, isolamento total entre clínicas etc. Rode
   `npm run check:claims:selftest` depois de editar a lista: se o detector não pegar as iscas, um
   "0 achados" não prova nada.
2. **O simulador da Luna não é escrito à mão.** `src/data/luna-cenarios.json` sai do motor real:
   ...
   O JSON grava o commit da Luna que o produziu. Se as regras de triagem mudarem, regenere.
...
| Detector de afirmações | self-test 10/10 iscas, 0 falso positivo; `dist/` com 0 achados; o protótipo antigo dispara 9 categorias |
